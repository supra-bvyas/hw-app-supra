import Transport from "@ledgerhq/hw-transport";
import SpeculosTransport from "@ledgerhq/hw-transport-node-speculos-http";
import TransportNodeHid from "@ledgerhq/hw-transport-node-hid";
import Supra from "./src/Supra";

async function exampleRaw(transport: Transport) {
  console.log("getVersion(raw)", await transport.send(0x5b, 0x03, 0x00, 0x00));
}

async function exampleSupra(transport: Transport) {
  const supraClinet = new Supra(transport);

  console.log("getVersion", await supraClinet.getVersion());
  console.log(
    "getAddress",
    await supraClinet.getAddress("m/44'/637'/1'/0'/0'", true)
  );

  const tx = Buffer.from(
    "",
    "hex"
  );
  console.log(
    "signTransaction",
    (
      await supraClinet.signTransaction("m/44'/637'/1'/0'/0'", tx)
    ).signature.toString("hex")
  );
}

const args = process.argv.slice(2);
const main = async (): Promise<void> => {
  const mode = args[0];
  let transport: Transport;
  switch (mode) {
    case "hid":
      transport = await TransportNodeHid.open(null);
      break;
    case "headless":
      transport = await SpeculosTransport.open({
        baseURL: "http://localhost:5000",
      });
      break;
    default:
      console.error(
        "Transport mode is not specified, available options: hid, headless"
      );
      return;
  }

  try {
    await exampleRaw(transport);
    await exampleSupra(transport);
  } catch (err) {
    console.log(err);
  } finally {
    await transport.close();
  }
};

main().catch((error) => {
  console.error(error);
  process.exit(1);
});
